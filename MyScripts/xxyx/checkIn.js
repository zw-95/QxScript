...
------------------------------------------
@Description:
晓晓优选 每天200+能量 能量兑换券，外卖返利
 
脚本兼容：Surge、QuantumultX、Loon、Shadowrocket、Node.js
只测试过QuantumultX，其它环境请自行尝试

2024.08.16 更新内容，重构代码，修复通知报错问题，支持多账号

重写:打开软件，点击我的

[rewrite_local]
^https:\/\/xxyx-client-api\.xiaoxiaoyouxuan\.com\/my url script-response-body https://raw.githubusercontent.com/zw-95/QxScript/master/MyScripts/xxyx/checkIn.js 
...
async function main() {
    for (let user of $.userList) {
        try {
            let pointF = await user.getUserEnergy();
            if (user.ckStatus) {
                await user.signIn();
                let taskList = await user.getTaskList();

                taskList = taskList.filter(e => e.isCompleted == 0);
                for (let task of taskList) {
                    for (let i = 0; i < task?.dailyCount; i++) {
                        await user.completeTask(task?.taskId);
                        $.log(`${task?.taskName}:调用成功!`);
                    }
                }
                let pointE = await user.getUserEnergy();
...
class UserInfo {
    constructor(user) {
        ...
        this.ckStatus = true;
        //请求封装
        this.baseUrl = `https://xxyx-client-api.xiaoxiaoyouxuan.com`;
        this.headers = {
            'xx-platform': `ios`,
        ...
        this.fetch = async (o) => {
            try {
                ...
                this.ckStatus = false;
                $.error(`[${this.userName || this.index}] ${e}`)
            }
        }
    }
    //签到
    ...
    async getTaskList() {
        try {
            const opts = {
                url: "https://xxyx-client-api.xiaoxiaoyouxuan.com/client/energy/mall/getTaskList?platform=ios",
            }
            let res = await this.fetch(opts);
            let taskList = res?.data.filter(e => e.taskName.match(/每日签到|分享海报|观看视频/));
            return taskList;
        } catch (e) {
            this.ckStatus = false;
            $.error(`[${this.userName || this.index}] ${e}`)
    ...
    async completeTask(taskId) {
        try {
            const opts = {
                url: `/client/energy/mall/completeTask/${taskId}`,
                type: "post",
                dataType: "json",
                body: { "taskId": taskId, "platform": "ios" }
            }
            return await this.fetch(opts);
        } catch (e) {
            this.ckStatus = false;
            $.error(`[${this.userName || this.index}] ${e}`)
        }
    ...
    async getUserEnergy() {
        try {
            const opts = {
                url: `/client/energy/mall/getUserEnergy?platform=ios`
            }
            let res = await this.fetch(opts);
