      ...
      if (body.code == 1 && body.records.length > 0) {
        ...
        if (posi) {
          ...
          const getPosiNameOptions = {
            ...
            body: $.toStr({ longitude: this.signRandomPosiLon, latitude: this.signRandomPosiLat }),
          }
          ...
          if (posiNameBody && posiNameBody.code > 0) {
            this.posiName =
              posiNameBody.addressName ||
              '湖北省武汉市江夏区关东街道光谷大道42号中国特种飞行器研发中心东南约' +
                (Math.floor(Math.random() * (10 - 3 + 1)) + 3) * 10 +
                '米'
      ...
      if (body) {
        if (body.code == 1) {
          ...
          var errorSignInRecords = body.records.filter(
            (v) =>
              ![...signInRecords, ...flowRecords, ...leaveRecords].includes(v) &&
              v.f2 != formatTimestamp(now.getTime() / 1000)
          )
        }
      }
      return {
