...
!(async () => {
  const cookie = $.getdata(ckName)
  const xAuthUser = $.getdata(xAuthUserName)
  const xAuthToken = $.getdata(xAuthTokenName)
  const userAgent = $.getdata(userAgentName)
  
  if (cookie && xAuthUser && xAuthToken && userAgent) {
    const msgContent = `Cookie: ${cookie.substring(0, 50)}...\nx-auth-user: ${xAuthUser}\nx-auth-token: ${xAuthToken.substring(0, 30)}...\nUser-Agent: ${userAgent.substring(0, 50)}...`
    $.msg($.name, '✅读取成功', msgContent)
    // 推送Bark通知
    await BarkNotify($, barkKey, $.name, msgContent)
    $.log(`会话参数:\nCookie: ${cookie}\nx-auth-user: ${xAuthUser}\nx-auth-token: ${xAuthToken}\nUser-Agent: ${userAgent}`)
  } else {
    const errMsg = `缓存中未找到完整参数:\nCookie: ${cookie ? '✅' : '❌'}\nx-auth-user: ${xAuthUser ? '✅' : '❌'}\nx-auth-token: ${xAuthToken ? '✅' : '❌'}\nUser-Agent: ${userAgent ? '✅' : '❌'}`
    $.msg($.name, '❌读取失败', errMsg)
    await BarkNotify($, barkKey, $.name, errMsg)
  }
  $.done()
})()
  .catch((e) => {
